...
for k in range(N+1):
    ...
    bump_steer = np.pi/36  # 5 degrees steering during bump
...
def car_patch(x, y, angle, width=0.1, length=0.3):
    car = patches.Rectangle((-length/2, -width/2), length, width, fill=False, edgecolor='r')
    t = plt.gca().transData
    rot = plt.matplotlib.transforms.Affine2D().rotate(angle)
    t_rot = t + rot
    car.set_transform(t_rot)
    car.set_xy((x, y))
    return car
...
# Plot small vehicle shapes
vehicle_spacing = max(1, N // 20)  # Adjust this value to change vehicle density
for i in range(0, N+1, vehicle_spacing):
    car = car_patch(x_res[i], y_res[i], delta_res[i])
    axs[2, 0].add_patch(car)
...
print(f"Maximum lateral deviation: {max(abs(y - y_r for y in y_res)):.2f} m")
print(f"Minimum speed: {min(v_x_res):.2f} m/s")
print(f"Maximum speed: {max(v_x_res):.2f} m/s")
print(f"Maximum steering angle: {max(abs(angle) for angle in delta_res):.2f} rad")