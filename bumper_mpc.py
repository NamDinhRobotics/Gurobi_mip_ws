...
v_desired = 15  # Desired speed to track (m/s)
...
lambda_jerk_base = 1  # Base weight for smooth jerk in the objective
lambda_jerk_max = 10  # Maximum weight for jerk near the bump
...
# Create Gurobi model
model = gp.Model("adaptive_jerk_longitudinal_control")
...
# Adaptive lambda_jerk variables
lambda_jerk = model.addVars(N, lb=lambda_jerk_base, ub=lambda_jerk_max, name="lambda_jerk")

# Auxiliary variables for quadratic terms
q_jerk = model.addVars(N, name="q_jerk")
...
model.addConstr(j[0] == 0, "initial_jerk")  # Assuming initial jerk is 0
...
for k in range(N):
    ...
    model.addConstr(a[k + 1] == a[k] + delta_t * j[k], name=f"dynamics_a_{k}")
...
# Adaptive lambda_jerk constraints
influence_range = 10  # meters before and after the bump where jerk weight increases
for k in range(N):
    model.addConstr(lambda_jerk[k] >= lambda_jerk_base +
                    (lambda_jerk_max - lambda_jerk_base) * (1 - (x[k] - speed_bump_min) / influence_range),
                    name=f"lambda_jerk_before_bump_{k}")
    model.addConstr(lambda_jerk[k] >= lambda_jerk_base +
                    (lambda_jerk_max - lambda_jerk_base) * (1 - (speed_bump_max - x[k]) / influence_range),
                    name=f"lambda_jerk_after_bump_{k}")

# Auxiliary constraints for quadratic terms
for k in range(N):
    model.addConstr(q_jerk[k] == j[k] * j[k], name=f"aux_q_jerk_{k}")
...
obj = lambda_smooth_tracking * gp.quicksum(a[k] * a[k] for k in range(N + 1)) + \
      ...
      gp.quicksum(lambda_jerk[k] * q_jerk[k] for k in range(N))
...
if model.status == GRB.OPTIMAL:
    ...
    lambda_jerk_sol = [lambda_jerk[k].X for k in range(N)]
    ...
    # Subplot 4: Adaptive Lambda Jerk vs Position
    plt.subplot(4, 1, 4)
    plt.plot(x_sol[:-1], lambda_jerk_sol, label='Adaptive Lambda Jerk', color='purple')
    plt.axvline(x=speed_bump_min, color='r', linestyle='--', label='Speed Bump Start')
    plt.axvline(x=speed_bump_max, color='r', linestyle='--', label='Speed Bump End')
    plt.fill_between([speed_bump_min, speed_bump_max], min(lambda_jerk_sol), max(lambda_jerk_sol), color='r', alpha=0.2, label='Speed Bump')
    plt.xlabel('Position (m)')
    plt.ylabel('Lambda Jerk')
    plt.title('Adaptive Lambda Jerk vs Position')
    plt.legend()
    plt.grid(True)
    plt.xlim(min(x_sol), max(x_sol))
